...
A drop-down groovebox for [Omarchy](https://omarchy.org). Press SUPER+L and a 16-step drum machine with a bass line and a lead synth slides down from the top of the screen, in your theme's colors. Make a loop. Press SUPER+L again and it slides away while the loop keeps playing.

No samples, no DAW, no config. Four drums and two synths are generated by a small Rust engine and go straight to PipeWire.

**Your theme has a sound.** The synth's filter, oscillator spread, drive, and sub weight are derived from the active theme's palette. Switch themes mid-loop and the sound changes with the colors.
...
`install.sh` builds the engine with cargo and, with `--bind`, appends a SUPER+L binding to `~/.config/hypr/bindings.lua`. Without `--bind` it prints the line for you to add yourself. If cargo is missing, the panel shows a Build button that runs the same build.
...
Omarchy 4 rebuilt the desktop as one Quickshell process and the community turned it into a plugin surface overnight. The plugins that spread are the ones that make the desktop do something new in the theme's own colors. Every music plugin so far plays other people's music. omaloop makes the desktop itself an instrument.
...
The panel reads the shell's live palette (`Color.accent`, `Color.background`) and sends one `tone` message to the engine whenever it changes:
...
Events come back as `{"event":"step","index":n}` on every step, `{"event":"state",...}` after `dump`, and `{"event":"exported","path":...}` when a render finishes. Tracks: `kick`, `snare`, `hat`, `ohat` (booleans) and `bass`, `lead` (MIDI notes, 0 = rest). Closed hat chokes the open hat.
